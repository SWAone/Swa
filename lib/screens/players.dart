...
class playrsScrren extends StatelessWidget {
  String bos;  String loction ,name ,docId ,png,mpng;
  
  

   playrsScrren({super.key,required this.mpng,required this.png,required this.bos,required this.loction,required this.name,required this.docId});
                          ...
                          SizedBox(height: 5.h,),
                           ...
                           ListView.separated(
                            ...
                            if (controller. pllyers.length>0) {
                              return
                              InkWell(
                                onTap: () => Get.to(()=>deilsPlya(docId: docId,playersId:controller.playersID[index] ,ppng: png,)),
                                child: ListTile(
                                trailing: Image(image: NetworkImage(png)),
                                subtitle: ineed.custmText(
                                
                                                          data: controller. pllyers[index]['markz'],
                                                          fontFamily: 'kufi',
                                                          isbold: false,
                                                          fontSize: 10.sp,
                                                          color: Colors.white),
                                leading: CircleAvatar(backgroundImage: NetworkImage(controller. pllyers[index]['imag']),radius: 30,backgroundColor: AppColor.TextColor,),
                                title: ineed.custmText(
                                
                                                          data:  controller. pllyers[index]['name'],
                                                          fontFamily: 'kufi',
                                                          isbold: true,
                                                          fontSize: 15.sp,
                                                          color: Colors.white),),
                           ...
                           }, itemCount: controller. pllyers.length>0?controller. pllyers.length:11)
                          ],
                        );
                         },
                      ),
